...
describe('getEdgeUrl', () => {
    ...
    ])("name: $name, undefinedEdge: $undefinedEdge", ({name, undefinedEdge}) => {
            const edgeStore = useEdgesStore()
            const edge: Edge | undefined = undefinedEdge ? undefined : { name: "name", hostname: "hostname", scheme: "http", port: 80n }
            vi.mocked(edgeStore.getEdge).mockReturnValue(edge)

            const url: string | undefined = getEdgeUrl(name)
            expect(url).toBe(name && edge ? `${edge.scheme}://${edge.hostname}:${edge.port}` : undefined)
